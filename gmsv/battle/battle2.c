		...
		for( i = 0; i < BATTLE_ENTRY_MAX; i ++ ){
			...
			if( CHAR_getInt( charaindex, CHAR_WHICHTYPE ) == CHAR_TYPEENEMY ){
				...
				if( BattleArray[battleindex].type == BATTLE_TYPE_P_vs_P ){
					...
					if( CHAR_getFlg( charaindex, CHAR_ISIP)) flg |= CHAR_FS_IP;		
#endif
...
int BATTLE_CreateVsPlayer( int charaindex0, int charaindex1 )
{
	...
	strcpysafe(BattleArray[battleindex].leadername,sizeof(BattleArray[battleindex].leadercdkey),CHAR_getChar(charaindex0,CHAR_NAME));
...
)
{
	...
	if(CHAR_getInt(charaindex, CHAR_FLOOR)==11111)vpmin = getBalltePoint(0)*2;
	if(CHAR_getInt(charaindex, CHAR_FLOOR)==22222)vpmin = getBalltePoint(1)*2;
	if(CHAR_getInt(charaindex, CHAR_FLOOR)==33333)vpmin = getBalltePoint(2)*2;
	if(CHAR_getInt(charaindex, CHAR_FLOOR)==44444)vpmin = getBalltePoint(3)*2;
...
void BATTLE_EscapeDpSend( int battleindex, int charaindex )
{
	...
	else if(CHAR_getInt( charaindex, CHAR_FLOOR)==44444)
		{
	...
	for( j = 0; j < 2; j ++ ){
		...
		for( i = 0; i < BATTLE_ENTRY_MAX; i ++ ){
			...
			if( enemytype == CHAR_TYPEPLAYER ||  enemytype == CHAR_TYPEENEMY	){
				int dpadd, dpnow;
				if(CHAR_getInt(enemyindex,CHAR_FLOOR) == 11111)dpadd = getBalltePoint(0);
				if(CHAR_getInt(enemyindex,CHAR_FLOOR) == 22222)dpadd = getBalltePoint(1);
				if(CHAR_getInt(enemyindex,CHAR_FLOOR) == 33333)dpadd = getBalltePoint(2);
				if(CHAR_getInt(enemyindex,CHAR_FLOOR) == 44444)dpadd = getBalltePoint(3);
				if(CHAR_getInt(enemyindex,CHAR_FLOOR)==11111||
					CHAR_getInt(enemyindex,CHAR_FLOOR)==22222||
					CHAR_getInt(enemyindex,CHAR_FLOOR)==33333||
					CHAR_getInt(enemyindex,CHAR_FLOOR)==44444)       //判定人物所在地图
					{
