...
//int BATTLE_ClearAmPoint( int charaindex )
//{
//	int i, pindex;
//
//	if( CHAR_CHECKINDEX( charaindex ) == FALSE ){
//		return BATTLE_ERR_CHARAINDEX;
//	}
//	CHAR_setWorkInt( charaindex, CHAR_WORKAMPOINT, 0 );
//	for( i = 0; i < CHAR_MAXPETHAVE; i ++ ){
//		pindex = CHAR_getCharPet( charaindex, i );
//		if( CHAR_CHECKINDEX( pindex ) == FALSE )continue;
//		CHAR_setWorkInt( pindex, CHAR_WORKAMPOINT, 0 );
//	}
//
//	return 0;
//}
...
int BATTLE_RescueEntry(	int charaindex, int toindex	)
{
	...
	//BATTLE_ClearAmPoint( charaindex );
	return iRet;
}
...
int BATTLE_DpCalc( int battleindex )   //PKDP
{
...
)
{
	...
	if(CHAR_getInt(charaindex, CHAR_FLOOR)==20001)vpmin = 1000;
	if(CHAR_getInt(charaindex, CHAR_FLOOR)==20002)vpmin = 5000;
	if(CHAR_getInt(charaindex, CHAR_FLOOR)==20003)vpmin = 10000;
	...
	//vpadd = CHAR_getWorkInt( charaindex, CHAR_WORKAMPOINT );
//	if(CHAR_getInt(charaindex, CHAR_FLOOR)==20001||
//		CHAR_getInt(charaindex, CHAR_FLOOR)==20002||
//		CHAR_getInt(charaindex, CHAR_FLOOR)==20003)
//  {
//  if( vpadd == 0 ){
//	vpadd = 0;					// 仇及桦宁反ㄟ
//	}else 
//		if( vpadd < 0 ){
//		vpadd = min( -1, vpadd );       //输了的
//	}else
//		{
//		vpadd = max( 1, vpadd );       //赢了的
//	}
//	vpnow = CHAR_getInt( charaindex, CHAR_AMPOINT ) + vpadd;      //PK会员点
//	vpnow = max( vpnow, 0 );          //计算现在会员点
//
//	CHAR_setInt( charaindex, CHAR_AMPOINT, vpnow );       //写入会员点数据
//	//CHAR_setInt( charaindex , CHAR_HP ,CHAR_getWorkInt( charaindex, CHAR_WORKMAXHP ) );   //恢复体力
//  //CHAR_setInt( charaindex , CHAR_MP ,CHAR_getWorkInt( charaindex, CHAR_WORKMAXMP ) );   //恢复气力
//	if(CHAR_getInt( charaindex, CHAR_AMPOINT)<vpmin)
//		{
...
//  }
	...
	if(CHAR_getInt(charaindex, CHAR_FLOOR)==20001||
		...
		CHAR_getInt(charaindex, CHAR_FLOOR)==20003)
  {
  	dpnow = CHAR_getInt( charaindex, CHAR_AMPOINT ) + dpadd;      //PK会员点
		dpnow = max( dpnow, 0 );          //计算现在会员点

		CHAR_setInt( charaindex, CHAR_AMPOINT, dpnow );       //写入会员点数据
		if(CHAR_getInt( charaindex, CHAR_AMPOINT)<vpmin)
		{
			CHAR_getElderPosition(CHAR_getInt(charaindex, CHAR_LASTTALKELDER), &fl, &x, &y);
			CHAR_warpToSpecificPoint(charaindex, fl, x, y);
	...
	for( j = 0; j < 2; j ++ ){
		...
		for( i = 0; i < BATTLE_ENTRY_MAX; i ++ ){
			...
			if( enemytype == CHAR_TYPEPLAYER ||  enemytype == CHAR_TYPEENEMY	){
				int dpadd, dpnow;
				//int vpadd, vpnow;
				if(CHAR_getInt(enemyindex,CHAR_FLOOR) == 20001)
					{
						dpadd = 1000;   //输了扣会员点
					}
				if(CHAR_getInt(enemyindex,CHAR_FLOOR) == 20002)
					{
						dpadd = 5000;
					}
				if(CHAR_getInt(enemyindex,CHAR_FLOOR) == 20003)
					{
						dpadd = 10000;
					}
					if(CHAR_getInt(enemyindex,CHAR_FLOOR)==20001||
						CHAR_getInt(enemyindex,CHAR_FLOOR)==20002||
						CHAR_getInt(enemyindex,CHAR_FLOOR)==20003)       //判定人物所在地图
						{
							dpnow = CHAR_getInt( enemyindex, CHAR_AMPOINT );
							dpnow -= dpadd;
...
//							if(CHAR_getInt(enemyindex,CHAR_FLOOR)==20001||
//								CHAR_getInt(enemyindex,CHAR_FLOOR)==20002||
//								CHAR_getInt(enemyindex,CHAR_FLOOR)==20003)
//								{
//							CHAR_setWorkInt( charaindex[k], CHAR_WORKAMPOINT, 
//								CHAR_getWorkInt( charaindex[k], CHAR_WORKAMPOINT) + vpadd );
//							}
					}
				}else{
