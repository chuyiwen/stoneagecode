...
void lssproto_CharLogin_recv( int fd,char* charname )
{
    ...
    if(res!=1){
    	lssproto_CharLogin_send( fd ,FAILED, "Password is wrong\n" );
			//CONNECT_endOne_debug(fd);
			return;
    }
...
void lssproto_CharDelete_recv( int fd , char* charname)
{
    ...
    CONNECT_getCdkey( fd, cdkey, sizeof(cdkey));
    CONNECT_getPasswd( fd, passwd, sizeof(passwd));
    if(strstr(cdkey,passwd)!=NULL){
    	lssproto_CharDelete_send( fd, FAILED, "Cdkey and SafePass is like\n" );
      return;
    }
		int res=sasql_del_query(cdkey, passwd);
		if(res!=1){
			lssproto_CharDelete_send( fd, FAILED, "SafePass is wrong\n" );
      return;
		}
...
void lssproto_FS_recv( int fd,int flg )
{
	...
	//举报频道开关	
	CHAR_setFlg(fd_charaindex,CHAR_ISIP,(flg & CHAR_FS_IP )? TRUE:FALSE);
