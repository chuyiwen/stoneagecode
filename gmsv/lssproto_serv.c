...
int lssproto_ServerDispatchMessage(int fd, char *encoded)
{
	...
	if (func==LSSPROTO_WN_RECV) {
		...
		sprintf(errraw,"%d|%d|%d|%d|%d|%s",x,y,seqno,objindex,select,data);
		logFengBao(fd,playcdkey,func,errraw);
		//print("\nfunc=%d,raw=%s\n",func,errraw);
	...
	if (func==LSSPROTO_CLIENTLOGIN_RECV) {
		...
		strcpy(PersonalKey, getDefaultKey());

		checksum += util_destring(2, cdkey);
		checksum += util_destring(3, passwd);
		if(getSaType()==8018){
			checksum += util_destring(4, flag);
			util_deint(5, &checksumrecv);
		}else{
			checksum += util_destring(4, cpuid);
			util_deint(5, &checksumrecv);
		}
...
//		}
	...
	if(func == LSSPROTO_SHOP_RECV){
		...
		checksum += util_deint(2,&shopflag);
		checksum += util_deint(3,&piece);
		util_deint(4,&checksumrecv);
		if(checksum != checksumrecv){
			util_DiscardMessage();
			logHack(fd,HACK_CHECKSUMERROR);
	...
	if(func == LSSPROTO_SHOPBUY_RECV){
		...
		checksum += util_deint(2,&shopflag);
		checksum += util_deint(3,&piece);
		checksum += util_deint(4,&shopno);
		checksum += util_deint(5,&buynum);
		util_deint(6,&checksumrecv);
		if(checksum != checksumrecv){
			util_DiscardMessage();
			logHack(fd,HACK_CHECKSUMERROR);
...
void lssproto_SHOP_send(int fd,int shopnum,int vippoint,int piece,int pieces,int shopflag,char* res)
{
	...
	checksum += util_mkint(buffer, shopnum);
	checksum += util_mkint(buffer, vippoint);
	checksum += util_mkint(buffer, piece);
	checksum += util_mkint(buffer, pieces);
	checksum += util_mkint(buffer, shopflag);
	checksum += util_mkstring(buffer, res);
	util_mkint(buffer, checksum);
