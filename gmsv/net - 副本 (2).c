...
void chardatasavecheck( void )
{
	...
	if( NowTime.tv_sec > (chardatasavecheck_store.tv_sec +10)){
		...
		for(i=0;i<maxnum;i++){
			if( CHAR_CHECKINDEX( i) && NowTime.tv_sec-CHAR_getWorkInt(i,CHAR_LASTCHARSAVETIME)>interval ){
				CHAR_setWorkInt(i,CHAR_LASTCHARSAVETIME,(int)NowTime.tv_sec);
...
#endif
#ifdef	_OFFLINE_SYSTEM
				if( getOfflineCf()==1 && CHAR_getWorkInt(charaindex,CHAR_WORK_OFFLINE) !=0 && CHAR_getWorkInt(charaindex,CHAR_WORKSTREETVENDOR)!=1){
					...
					if ( CHAR_getWorkInt( Connect[ i ].charaindex, CHAR_WORK_OFFLINE )==0){
						if( strlen(CHAR_getChar(Connect[ i ].charaindex,CHAR_OFFBUF))<1){
							...
							if(mymon!=mon || mydate!=date){
								...
								if(mycnt<12 && timep-Connect[ i ].starttime >= 600 && CHAR_getInt(Connect[ i ].charaindex,CHAR_OFFTIME)<86400){
...
//			else
			{
				...
				if (cono_check&CONO_CHECK_ITEM)
					...
					if (cono_check&CONO_CHECK_PET)
						...
						if((cono == 0) || (acceptmore <= 0) || isThereThisIP( sinip) || isManRen()){
							// Nuke +2 Errormessage
