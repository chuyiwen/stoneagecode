	...
	if( checkT != NowTimes && (checkT+10) <= NowTimes )	{
		...
		for ( i = 0;i < ConnectLen; i++ ) {
			if ( ( Connect[ i ].use ) && ( i != acfd )) {
...
#ifdef _ITEM_ADDEXP	//vincent 经验提升
					if ( CHAR_getInt( Connect[ i ].charaindex, CHAR_ADDEXPPOWER ) > 0 &&
...
#else
							if ( Connect[ i ].EDTime < CHAR_getInt( Connect[ i ].charaindex, CHAR_ADDEXPTIME ) ) { //功能作用有效期限
								Connect[ i ].EDTime = Connect[ i ].EDTime + 300;
							}else{
								Connect[ i ].EDTime = 0;
								CHAR_setInt( Connect[ i ].charaindex, CHAR_ADDEXPPOWER, 0 );
								CHAR_setInt( Connect[ i ].charaindex, CHAR_ADDEXPTIME, 0 );
				...
				if ( CHAR_getInt( Connect[ i ].charaindex, CHAR_VIPRIDE ) > 0 && CHAR_getInt( Connect[ i ].charaindex, CHAR_VIPTIME ) > 0 ){
					time_t timep;
					struct tm *p;
					time(&timep);
					p=localtime(&timep);
					timep = mktime(p);
					if(CHAR_getInt( Connect[ i ].charaindex, CHAR_VIPTIME )<timep){
						CHAR_setInt(Connect[ i ].charaindex, CHAR_VIPRIDE,0);
						CHAR_setInt(Connect[ i ].charaindex, CHAR_VIPTIME,0);
						CHAR_loginCheckUserItem(Connect[ i ].charaindex);
					}
				}
...
//			else
			{
				...
				if (cono_check&CONO_CHECK_ITEM)
					if ((total_item_use+500) >= MAX_item_use){
						print("可使用物品数已满!!");
						cono=0;
					}
					if (cono_check&CONO_CHECK_PET)
						if( (petcnt+500) >= CHAR_getPetMaxNum() ){
							print("可使用宠物数已满!!");
							cono=0;
						}
	...
	default:
		{
			...
			if ( i_counter > 10 ) { //10秒
				...
				}
				{
					...
					*/
					if((petcnt+300)>=getPetcharnum() || (total_item_use+300)>=getItemnum()){
...
LUCKBEGIN:
	...
	default:
		...
		if ( Connect[ fdremember ].state == NOTLOGIN && Connect[ fdremember ].ctype!=CLI ){
	    	if ( fdremember != acfd ){
