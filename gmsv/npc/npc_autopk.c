								...
								int select, char *data)
{
	...
	case NPC_AUTOPK_JOINPK:
		if(autopktime>2
			...
			CHAR_warpToSpecificPoint( talkerindex, 20000, 15, 15 );
#ifdef _GMRELOAD
...
#ifdef	_YUANGUSA
				sprintf(buf1,"玩家%s参加每日乱舞PK活动！",CHAR_getChar( talkerindex, CHAR_NAME));
				sprintf(buf2,"玩家%s参加每日乱舞PK活动,有兴趣来挑战他么？报名地址：玛丽娜斯渔村(80.70)-PK系统管理员，奖励大量积分哦！",CHAR_getChar( talkerindex, CHAR_NAME));
#else				
...
void AutoPk_PKSystemInfo()
{
	...
	if(autopktime>0){
		...
		char buf2[128];
#ifdef	_YUANGUSA
		sprintf(buf1, "距离参加每日乱舞PK大赛时间还剩%d分钟，请大家提前入场！",autopktime);
		if(awardpetid!=-1 && awarditemid==-1)
			sprintf(buf2,"距离参加每日乱舞PK大赛时间还剩%d分钟，请大家提前入场，本次比赛奖品为:编号:%d,宠物:%s",autopktime-2,awardpetid,awardpet);
		else if(awardpetid==-1 && awarditemid!=-1)
			sprintf(buf2,"距离参加每日乱舞PK大赛时间还剩%d分钟，请大家提前入场，本次比赛奖品为:编号:%d,物品:%s",autopktime-2,awarditemid,awarditem);
		else if(awardpetid!=-1 && awarditemid!=-1)
			sprintf(buf2,"距离参加每日乱舞PK大赛时间还剩%d分钟，请大家提前入场，本次比赛奖品为:编号:%d,宠物:%s和编号:%d,物品:%s",autopktime-2,awardpetid,awardpet,awarditemid,awarditem);
		else
			sprintf(buf2, "距离每日乱舞PK大赛时间还剩%d分钟，请大家提前入场！",autopktime);
#else
...
void AutoPk_GetChampionShip( void )
{
	...
	if(strlen(ship[0].cdkey)>0){
		sprintf( token, "恭喜%s获得本次PK比赛冠军！", ship[0].name);
		AutoPk_PKSystemTalk(token,token);
		AutoPk_PKTimeSetOnly(-1);
		autopkstart=0;
	}
	if(strlen(ship[1].cdkey)>0){
		sprintf( token, "恭喜%s获得本次PK比赛亚军！", ship[1].name);
		AutoPk_PKSystemTalk(token,token);
	}
	if(strlen(ship[2].cdkey)>0){
		sprintf( token, "恭喜%s获得本次PK比赛季军！", ship[2].name);
		AutoPk_PKSystemTalk(token,token);
	}
...
void NPC_AUTOPKHealerTalked( int meindex )
{
  if( CHAR_getInt( meindex , CHAR_WHICHTYPE ) != CHAR_TYPEPLAYER )	return;
	...
	|| (CHAR_getWorkInt( meindex, CHAR_WORKPARTYMODE) == 2) )
	{
		...
		CHAR_talkToCli( meindex, -1,"恭喜你获胜，请等待本轮比赛结束，体力已全部回复。",CHAR_COLORBLUE);
	}else{
		int i = 0;
		int otherindex;
