...
//		}
		...
		if(randtype==1){
			...
			for(n=1;n<=5;n++){
				if(n==randnum1){
					sprintf(tempret[n-1],"%d、%s",rightnum,randrightanswer);
				}else{
					if(m==1){
						sprintf(tempret[n-1],"%d、%s",wrongnum1,randwronganswer1);
						m = 2;
					}else if(m==2){
						sprintf(tempret[n-1],"%d、%s",wrongnum2,randwronganswer2);
						m = 3;
					}else if(m==3){
						sprintf(tempret[n-1],"%d、%s",wrongnum3,randwronganswer3);
						m = 4;
					}else if(m==4){
						sprintf(tempret[n-1],"%d、%s",wrongnum4,randwronganswer4);
						m = 5;
					}
				}
			}
			...
			CHAR_setWorkChar(talkerindex, CHAR_WORKRANDMSG, buf);
			int fd = getfdFromCharaIndex( talkerindex );
			sprintf(buf,"%s：\n%s\n%s\n%s\n%s\n%s\n输入正确答案前的数字",randquestion,tempret[0],tempret[1],tempret[2],tempret[3],tempret[4]);
			lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGEANDLINEINPUT, 
				WINDOW_BUTTONTYPE_OK,
