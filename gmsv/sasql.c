...
int sasql_query(char *nm, char *pas){
	...
	if(!mysql_query(&mysql,sqlstr)){
	  ...
	  if(num_row>0){
	  	...
	  	if(mysql_row[2]!=NULL){
	  		if((strcmp(pas,mysql_row[1]) == 0 || strcmp(pas,mysql_row[2]) == 0) && atoi(mysql_row[3])==0){
	  		return 1;
		  	}else{
		  		//printf("用户%s密码错误！\n",nm);
		  		return 2;
...
BOOL sasql_online_jqm( char *ID, char *jqm, char *IP )
{
	if(openbackground){
		char sqlstr[256];
		sprintf(sqlstr,"update `%s` set `LoginTime`=NOW(),`IP`='%s',`PcId`='%s', `Online`=%d where `%s`=BINARY'%s'", sqlconfig.sql_Table, IP,jqm, getServernumber(), sqlconfig.sql_NAME, ID);
		if(!mysql_query(&mysql,sqlstr)){
			return TRUE;
		}
	}
	  			...
	  			if(check == 1)
	  				{
	  					...
	  					if(!mysql_query(&mysql,sqlstr)){
	  						sprintf(sqlstr,"%d会员点冲值成功，您现在的会员点数为：%d",costval,sasql_vippoint(id,costval,1));
...
int sasql_cdkey_lock(char *cdkey){
	...
	if(!mysql_query(&mysql,sqlstr)){
		...
		if(num_row>0){
			mysql_row=mysql_fetch_row(mysql_result);
			mysql_free_result(mysql_result);
			sprintf(sqlstr,"update `%s` set `Lock`=1 where `%s`=BINARY'%s'", sqlconfig.sql_Table,sqlconfig.sql_NAME, cdkey);	
			if(!mysql_query(&mysql,sqlstr)){
				return 1;
			}else{
				return -1;
