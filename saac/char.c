...
static void makeCharBakFileName( char *idstring ,char *output, int outlen,int num);
// Nuke *1*1
...
static void makeCharBakFileName( char *id,char *output, int outlen, int num)
{
	char body[1024];
	if ( strlen( id) < 1)return;
	struct tm *now;
	...
	time( &timenow );
	now = localtime( &timenow );
	snprintf(body , sizeof(body ) , "%s.%d.char-%d%d%d%d%d%d" , id , num,now->tm_year + 1900,now->tm_mon + 1 , now->tm_mday ,now->tm_hour , now->tm_min , now->tm_sec );
	makeDirFilename(output , outlen, charbakdir , getHash(id) ,body);
}
...
static int unlinkCharFile( char *id , int num )
{
	...
	makeCharBakFileName( id, bakname, sizeof(bakname),num );
	return rename( fname,bakname );
}
