...
void saacproto_ACCharDelete_recv( int ti,char* id,char* pas,
                                  char* charname , char *option , int mesgid, char *userip)
{
    ...
    // Nuke
    charDeleteCallback(ti,0,id,pas,charname,"0","",mesgid,0,userip);
    log( "档案删除: 附加作业 %s %s\n",id , pas );
}
...
void saacproto_ACNewDelFM_recv(int fd, int index, int charfdid)
{
	...
	if (r < 0)
		...
		saacproto_ACDelFM_send(fd, SUCCESSFUL, charfdid);
	*/   
	int fmindex = getfmindex(index);
	char *fmname = getfmname(index);
	r = ACDelFM(index, fmname, fmindex); 	
	if (r < 0)
		saacproto_ACDelFM_send(fd, FAILED, charfdid);
	else
...
void saacproto_ACSystemAllTalk_recv( int fd, char *data)
{
	...
	for (i=0; i<MAXCONNECTION; i++) {
		if (gs[i].use && gs[i].name[0]) {
				saacproto_ACSystemAllTalk_send(i, data);
		}
	}
}
...
void saacproto_ACPlayerAllTalk_recv( int fd, char *servername,char *playname,char *data)
{
	...
	for (i=0; i<MAXCONNECTION; i++) {
		if (gs[i].use && gs[i].name[0]) {
				saacproto_ACPlayerAllTalk_send(i, servername,playname,data);
		}
	}
}
