...
int saacproto_ServerDispatchMessage( int fd , char *encoded, char *debugfun)
{
	...
	if( strcmp( funcname , "ACCharLoad" ) == 0 ){
		...
		ip = saacproto_wrapStringAddr( saacproto_stringwrapper[7] , saacproto.workbufsize , saacproto_demkstr_string( saacproto.token_list[8] ));
	...
	if( strcmp( funcname , "ACCharDelete" ) == 0 ){
		...
		userip = saacproto_wrapStringAddr( saacproto_stringwrapper[6] , saacproto.workbufsize , saacproto_demkstr_string( saacproto.token_list[7] ));
		saacproto_ACCharDelete_recv( fd,id,passwd,charname,option,mesgid,userip);
		return 0;
	}
	...
	if ( strcmp( funcname, "ACNewDelFM") == 0){
		int 	index, charfdid;
		index = saacproto_demkstr_int(saacproto.token_list[2]);
		charfdid = saacproto_demkstr_int(saacproto.token_list[3]);
		saacproto_ACNewDelFM_recv(fd, index, charfdid);
		return	0;
	}
	
	if ( strcmp( funcname, "ACSystemAllTalk") == 0){
		char *bufdata;
		bufdata = saacproto_wrapStringAddr( saacproto_stringwrapper[1] , saacproto.workbufsize , saacproto_demkstr_string( saacproto.token_list[2] ));
		saacproto_ACSystemAllTalk_recv(fd, bufdata);
		return	0;
	}
	...
	if ( strcmp( funcname, "ACPlayerAllTalk") == 0){
		...
		char *bufdata;
		servername = saacproto_wrapStringAddr( saacproto_stringwrapper[1] , saacproto.workbufsize , saacproto_demkstr_string( saacproto.token_list[2] ));
		playname = saacproto_wrapStringAddr( saacproto_stringwrapper[2] , saacproto.workbufsize , saacproto_demkstr_string( saacproto.token_list[3] ));
		bufdata = saacproto_wrapStringAddr( saacproto_stringwrapper[3] , saacproto.workbufsize , saacproto_demkstr_string( saacproto.token_list[4] ));
		saacproto_ACPlayerAllTalk_recv(fd, servername,playname,bufdata);
		return	0;
	}
...
void saacproto_ACSystemAllTalk_send( int fd, char *data )
{
...
void saacproto_ACPlayerAllTalk_send( int fd,char *servername,char *playname, char *data )
{
